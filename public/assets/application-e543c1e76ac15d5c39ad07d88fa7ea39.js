...
(function() {
  $(function() {
    return $(".next-chapter").on("click", function(e) {
      var $target, target;
      e.preventDefault();
      target = this.hash;
      $target = $(target);
...
(function() {
  this.orderTotal = function() {
    var gross, net_price, order_price, price, quantity;
    quantity = $('#order_quantity').val();
    price = $('.price').data('price');
    net_price = quantity * price;
    gross = (net_price / .961) + 0.30;
    order_price = price > 0 ? gross : 0.0;
    return $('.order-total').html(order_price.toFixed(2));
  };

  $(function() {
    return $('.best_in_place_about').on('ajax:success', function() {
      return $.get(window.location.pathname, function(data) {
        return $('.about').html($(data).find(".about").html());
      });
    });
  });

  $(function() {
    return $('.edit-about').click(function() {
      $(this).text(function() {
        if ($(this).is('.active')) {
          return "Edit";
        } else {
      ...
      return $('#cancel-button').toggleClass("hideme");
    });
  });

  $(function() {
    return $('#cancel-button').on('mousedown', function() {
      $('.edit-about').removeClass('active').text("Edit");
      return $(this).addClass("hideme");
    });
  });

  $(function() {
    return $(document).keyup(function(e) {
      if (e.keyCode === 27 && $('.edit-about').is('.active')) {
        $('.edit-about').removeClass('active').text("Edit");
        return $('#cancel-button').addClass("hideme");
      }
    });
  });
  ...
  $(function() {
    orderTotal();
    return $('#order_quantity').change(function() {
      return orderTotal();
    });
  });
  ...
  $(function() {
    if ($("#event-banner").data('url')) {
      return $("#event-banner").backstretch($("#event-banner").data('url'));
    } else {
      return $("#event-banner").hide();
    }
  });

  $(function() {
    var widthSold, widthUnsold;
    widthSold = $('.progress-bar').data('funded');
    widthUnsold = 100 - $('.progress-bar').data('funded');
    $('.sold').css("width", "" + widthSold + "%");
    return $('.unsold').css("width", "" + widthUnsold + "%");
  });

  $(function() {
    var handler;
    handler = StripeCheckout.configure({
      key: "pk_51Rs538eA9U9NR6KAbydxKg92wQLy",
      image: "/assets/motels_m.png",
      token: function(response) {
        var emailInput, tokenInput;
        ...
        return $("form").append(tokenInput).append(emailInput).submit();
      }
    });
    return $("#checkout").click(function(e) {
      handler.open({
        address: false,
